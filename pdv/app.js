...
// ==========================================
// 📡 RADAR GLOBAL, ÁUDIO DESTREVADO E IMPRESSÃO AUTÔNOMA
// ==========================================
let ultimoIdVendaConhecido = 0; // Inteligência alterada: rastreia a ID exata e não apenas a quantidade
const audioAlertaGlobal = new Audio('https://www.myinstants.com/media/sounds/ding-sound-effect_2.mp3');

// 🚀 O DESTRAVADOR DE ÁUDIO DO GOOGLE CHROME
// No primeiro clique que você der em qualquer lugar da tela, ele libera a permissão de tocar som no fundo
document.body.addEventListener('click', () => {
    if (audioAlertaGlobal.paused) {
        audioAlertaGlobal.play().then(() => {
            audioAlertaGlobal.pause();
            audioAlertaGlobal.currentTime = 0;
        }).catch(() => {});
    }
}, { once: true });
...
async function checarNovosPedidosGlobal() {
    try {
        const API_URL_RADAR = 'http://108.174.146.77:3000/api'; 
        const res = await fetch(`${API_URL_RADAR}/vendas`);
        const vendas = await res.json();
        
        const pendentes = vendas.filter(v => v.status && v.status.trim() === 'Pendente Delivery');

        // Primeira vez que o sistema abre: ele só memoriza qual é o último pedido para não imprimir o passado
        if (ultimoIdVendaConhecido === 0) {
            if (vendas.length > 0) {
                ultimoIdVendaConhecido = Math.max(...vendas.map(v => v.id));
            } else {
                ultimoIdVendaConhecido = -1;
            }
            return; 
        }

        // Acha exatamente QUAIS pedidos acabaram de chegar
        const novosPedidos = pendentes.filter(v => v.id > ultimoIdVendaConhecido);
        ...
        if (novosPedidos.length > 0) {
            ...
            audioAlertaGlobal.play().catch(e => console.log("Áudio bloqueado. Lembre-se de clicar na tela uma vez ao abrir o painel."));
            
            // 2. Mostra a bolha visual
            mostrarAlertaVisualDelivery();

            // 3. Notificação Multicanal (Windows/Mac)
            if ("Notification" in window && Notification.permission === "granted") {
                const notificacao = new Notification("🚨 NOVO PEDIDO - ICESOFT", {
                    body: `Chegou pedido novo! Clique para abrir.`,
            ...
            // 🖨️ 4. A MÁGICA: Manda imprimir cada pedido novo automaticamente
            novosPedidos.forEach(pedido => {
                imprimirComandaDelivery(pedido);
            });
        }
    } catch (e) {}
}
...
// ==========================================
function imprimirComandaDelivery(venda) {
    ...
    try {
        ...
        itens.forEach(item => {
            itensHtml += `<div style="font-size: 15px; margin-bottom: 6px; font-weight: bold;">1x ${item.nome.replace('Delivery: ', '')} <span style="float: right;">R$ ${Number(item.preco).toFixed(2).replace('.', ',')}</span></div>`;
        });
    } catch(e) {}
    ...
    const conteudoImpressao = `
        ...
        <head>
            <style>
                body { font-family: 'Courier New', Courier, monospace; width: 75mm; margin: 0 auto; padding: 5px; color: #000; }
                h2 { text-align: center; margin: 0 0 5px 0; font-size: 20px; text-transform: uppercase; }
                .linha { border-bottom: 2px dashed #000; margin: 10px 0; }
                .destaque { font-size: 16px; font-weight: bold; margin-bottom: 5px; }
                .info-cliente { font-size: 15px; margin-bottom: 4px; }
            </style>
        </head>
        <body>
            ...
            <div class="info-cliente"><strong>Celular:</strong> ${venda.cliente_telefone || 'Não informado'}</div>
            
            <div class="linha"></div>
            <div class="destaque">ENDEREÇO DE ENTREGA:</div>
            <div style="font-size: 16px; font-weight: bold; line-height: 1.3;">${venda.cliente_endereco || 'Retirada na Loja'}</div>
            
            <div class="linha"></div>
            <div class="destaque">ITENS DO PEDIDO:</div>
            ${itensHtml}
            
            <div class="linha"></div>
            <div class="destaque" style="text-align: right; font-size: 18px;">TOTAL: R$ ${Number(venda.valor_total).toFixed(2).replace('.', ',')}</div>
            <div style="text-align: right; font-size: 14px;"><strong>Pagamento:</strong> ${venda.forma_pagamento}</div>
            
            ${venda.observacoes ? `<div class="linha"></div><div class="destaque">OBSERVAÇÕES:</div><div style="font-size: 16px; font-weight: bold; border: 2px solid #000; padding: 5px;">${venda.observacoes}</div>` : ''}
            
            <div class="linha"></div>
            <div style="text-align: center; font-size: 12px; margin-top: 15px;">Icesoft - O melhor da região!</div>
        </body>
        </html>
    `;
    ...
    doc.open();
    doc.write(conteudoImpressao);
    doc.close();

    setTimeout(() => {
        iframe.contentWindow.focus();
        iframe.contentWindow.print();
    }, 800); // Dá 800ms pro navegador desenhar a tela invisível antes de chamar a impressora
}
